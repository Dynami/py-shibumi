...
def EMA(df, n):
    EMA = pd.Series( df['close'].ewm(ignore_na=False, span=n ,min_periods=(n-1) , adjust=True).mean(), name = 'EMA_' + str(n))
...
def myRSI(df, n):
    ...
    up_move = pd.Series(np.where(move > 0, move, 0))
    dwn_move = pd.Series(np.where(move < 0, -move, 0))
    
    up_move_avg = up_move.rolling(window=n).mean()
    dwn_move_avg = dwn_move.rolling(window=n).mean()
    RS = pd.Series(up_move_avg/dwn_move_avg)
    RSI = pd.Series((1 - 1./(1.+RS)), name = 'RSI_' + str(n))
    RSI.index = df.index
    df = df.join(RSI)
    return df
...
def RSI(df, n):
    ...
    while i + 1 < df.shape[0]:
        UpMove = df['high'].iat[i] - df['high'].iat[i-1]
        print(UpMove)
        
#         UpMove = df.get_value(i + 1, 'high') - df.get_value(i, 'high')
        DoMove = df['low'].iat[i-1] - df['low'].iat[i]
#         DoMove = df.get_value(i, 'low') - df.get_value(i + 1, 'low')
        if UpMove > DoMove and UpMove > 0:
            UpD = UpMove
        else: UpD = 0
...
def STDDEV(df, n):
    ...
    df = df.join(pd.Series(df['close'].rolling(center=False, window=n).std(), name='STD_' + str(n)))
    return df